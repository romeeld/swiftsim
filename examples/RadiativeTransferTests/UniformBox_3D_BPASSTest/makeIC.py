...
w.gas.masses = np.ones(xp.shape[0], dtype=np.float64) * 125 * unyt.Msun
w.stars.masses = np.ones(xs.shape[0], dtype=np.float64) * 125 * unyt.Msun
# w.stars.birth_times = np.zeros(xs.shape[0], dtype=np.float64) * unyt.Myr
w.gas.internal_energy = (
    np.ones(xp.shape[0], dtype=np.float64) * (300.0 * unyt.kb * unyt.K) / (125 * unyt.Msun)
)
...
# Set up star metallicities
# Here we consider stars have same metallicity
Metallicity = 0.02
...
sparts = F["/PartType4"]
dsetname = "StellarFormationTime"
stardata = np.zeros(nstars, dtype=np.float64)
sparts.create_dataset(dsetname, data=stardata)

dsetname = "StellarMetallicity"
metaldata = Metallicity * np.ones(nstars, dtype=np.float64)
sparts.create_dataset(dsetname, data=metaldata)
