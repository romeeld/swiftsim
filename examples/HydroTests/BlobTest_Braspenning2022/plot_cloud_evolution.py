...
def analyse_snapshot(i, chi, rho_cl0, m_cl0, resolution):
    ...
    print(bins_x)
    binned_masses, _ = np.histogramdd(data.gas.coordinates.v, weights=data.gas.masses.v, bins = [bins_x, bins_y, bins_z])
    ...
    if chi == 100:
        m_t_intermediate = np.sum(data.gas.masses.value[(data.gas.temperatures > 10**4.5) & (data.gas.temperatures < 10**5.5)]) / m_cl0
    elif chi == 10:
        m_t_intermediate = np.sum(data.gas.masses.value[(data.gas.temperatures > 10**4.25) & (data.gas.temperatures < 10**4.75)]) / m_cl0
    ...
    return t, dense_mass, m_t_intermediate, dense_mass_resampled_16, m_t_intermediate_resampled_16, dense_mass_resampled_32, m_t_intermediate_resampled_32
...
def load_first_snapshot(chi, resolution):
    ...
    v_wind0 = data_0.gas.velocities[0, 0] #All wind particles have the same initial velocity
    r_cl0 = data_0.metadata.boxsize[1] * 0.1 #The cloud is 0.1 * boxsize

    rho_cl0 = chi * np.percentile(data_0.gas.densities.value, 50)
    cloud_0 = data_0.gas.densities.value > rho_cl0/3 #Create a mask for the dense gas
...
for i in range(steps):
    time[i], dense_mass[i], intermediate_temp_mass[i], dense_mass_16[i],\
    intermediate_temp_mass_16[i], dense_mass_32[i], intermediate_temp_mass_32[i] = \
