...
__attribute__((always_inline)) INLINE float cooling_compute_G0(
  ...
  if (cooling->G0_computation_method==0) {
    ...
    pssfr /= max(p->group_data.stellar_mass, 8. * p->mass);
    G0 = max(p->group_data.ssfr, pssfr) * cooling->G0_factor2 + 
...
__attribute__((always_inline)) INLINE void firehose_cooling_and_dust(
  ...
  /* Check to make sure it is actually a wind particle */
...
#ifdef FIREHOSE_DEBUG_CHECKS
  message("FIREHOSE_COOLING: id=%lld rho=%g u=%g rhoamb=%g uamb=%g tcool=%g",
          p->id, 
          rho_old, 
          u_old,
...
void cooling_cool_part(const struct phys_const* restrict phys_const,
  ...
  if (p->cooling_data.subgrid_temp == 0.) {  
    ...
    /* Evolve the subgrid cold ISM fraction for particle: 
