...
__attribute__((always_inline)) INLINE static void cooling_debug_particle(
  ...
  printf(
      "cooling_xpart_data:\nradiated_energy = %.3e, time_last_event = %.3e\n",
      xp->cooling_data.radiated_energy, xp->cooling_data.time_last_event);

#if COOLING_GRACKLE_MODE >= 1
  printf(
      "HI_frac = %.3e, HII_frac = %.3e, HeI_frac = %.3e, HeII_frac = %.3e, "
      "HeIII_frac = %.3e, e_frac = %.3e\n",
      xp->cooling_data.HI_frac, xp->cooling_data.HII_frac,
      xp->cooling_data.HeI_frac, xp->cooling_data.HeII_frac,
      xp->cooling_data.HeIII_frac, xp->cooling_data.e_frac);
#if COOLING_GRACKLE_MODE >= 2
  printf("HM_frac = %.3e, H2I_frac = %.3e, H2II_frac = %.3e\n",
         xp->cooling_data.HM_frac, xp->cooling_data.H2I_frac,
         xp->cooling_data.H2II_frac);
#if COOLING_GRACKLE_MODE >= 3
  printf("DI_frac = %.3e, DII_frac = %.3e, HDI_frac = %.3e\n",
         xp->cooling_data.DI_frac, xp->cooling_data.DII_frac,
         xp->cooling_data.HDI_frac);
#endif
#endif
#endif
  printf("metal_frac = %.3e\n", xp->cooling_data.metal_frac);
}
