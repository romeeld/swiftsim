...
__attribute__((always_inline)) INLINE static void black_holes_prepare_feedback(
  ...
  if (galaxy_gas_mass > 0) {
    f_corr_stellar = 
        min(1. + bp->group_data.stellar_mass / galaxy_gas_mass, 
            f_corr_stellar);
  }
...
#ifdef SIMBA_DEBUG_CHECKS
  double f_gas = bp->group_data.gas_mass / 
      (bp->group_data.stellar_mass + bp->group_data.gas_mass);
