 ...
 *
 * @param r2 Comovin square distance between the two particles.
 * @param dx Comoving vector separating both particles (pi - pj).
 * @param bi first particle (black hole).
 * @param sj Second particle (stars, not updated).
 */
__attribute__((always_inline)) INLINE static void
runner_iact_nonsym_bh_stars_density(const float r2, const float dx[3], struct bpart* bi, const struct spart* sj) { }
 ...
 */
__attribute__((always_inline)) INLINE static void
runner_iact_nonsym_bh_stars_bulge(const float r2, const float dx[3], struct bpart* bi, const struct spart* sj) { }
...
runner_iact_nonsym_bh_gas_swallow(
  ...
  if (bi->group_data.stellar_mass <= 0.f) return;
...
runner_iact_nonsym_bh_gas_feedback(
  ...
  if (pj->black_holes_data.swallow_id != bi->id) {
    ...
    if (bi->v_kick > bh_props->xray_heating_velocity_threshold) {
      const float group_mass = bi->group_data.mass;

      float f_gas = 0.f;
      if (group_mass > 0.f) {
        f_gas = 1.f - bi->group_data.stellar_mass / group_mass;
      }
