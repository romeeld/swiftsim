 ...
 *
 ******************************************************************************/
#ifndef SWIFT_ENTROPY_FLOOR_SIMBA_H
#define SWIFT_ENTROPY_FLOOR_SIMBA_H
...
/**
 * @file src/entropy_floor/SIMBA/entropy_floor.h
 * @brief Entropy floor used in the SIMBA model
 */

/**
 * @brief Properties of the entropy floor in the SIMBA model.
 */
...
static INLINE float entropy_floor_gas_pressure(
  ...
  /* Are we in the regime of the Jeans equation of state (ISM densities)? */
  ...
  /* Are we in the regime of the Cool equation of state (sub-ISM densities)? */
  if ((rho_com >= rho_crit_baryon * props->Cool_over_density_threshold) &&
      (rho_com < rho_crit_baryon * props->Jeans_over_density_threshold) &&
...
static INLINE void entropy_floor_init(struct entropy_floor_properties *props,
  ...
  props->Jeans_density_threshold_H_p_cm3 = parser_get_param_float(
      params, "SIMBAEntropyFloor:Jeans_density_threshold_H_p_cm3");
  props->Jeans_over_density_threshold = parser_get_param_float(
      params, "SIMBAEntropyFloor:Jeans_over_density_threshold");
  props->Jeans_temperature_norm_K = parser_get_param_float(
      params, "SIMBAEntropyFloor:Jeans_temperature_norm_K");
  props->Jeans_gamma_effective =
      parser_get_param_float(params, "SIMBAEntropyFloor:Jeans_gamma_effective");

  props->Cool_density_threshold_H_p_cm3 = parser_get_param_float(
      params, "SIMBAEntropyFloor:Cool_density_threshold_H_p_cm3");
  props->Cool_over_density_threshold = parser_get_param_float(
      params, "SIMBAEntropyFloor:Cool_over_density_threshold");
...
static INLINE void entropy_floor_print(
  ...
  message("Entropy floor is 'SIMBA' with:");
...
INLINE static void entropy_floor_write_flavour(hid_t h_grp) {
  ...
  io_write_attribute_s(h_grp, "Entropy floor", "SIMBA");
}
#endif
...
#endif /* SWIFT_ENTROPY_FLOOR_SIMBA_H */
